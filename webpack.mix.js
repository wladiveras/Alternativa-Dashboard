...
mix.sass('resources/plugins/plugins.scss', CompilerPath + 'plugins/global/plugins.bundle.css').then(() => {
    // remove unused preprocessed fonts folder
    rimraf(path.resolve('public/fonts'), () => {});
    rimraf(path.resolve('public/images'), () => {});
}).sourceMaps(!mix.inProduction())
    // .setResourceRoot('./')
    .options({processCssUrls: false}).js(['resources/plugins/plugins.js'], CompilerPath + 'plugins/global/plugins.bundle.js');

// wladi-workspace css/js
//mix.sass('resources/wladi-workspace/sass/style.scss', CompilerPath + 'css/style.bundle.css', {
//    sassOptions: {includePaths: ['node_modules']},
//})
//    .options({processCssUrls: false})
//    .js('resources/js/scripts.js', CompilerPath + 'js/scripts.bundle.js');

// Custom 3rd party plugins
(glob.sync('resources/plugins/custom/**/*.js') || []).forEach(file => {
    mix.js(file, CompilerPath + `${file.replace('resources/', '').replace('.js', '.bundle.js')}`);
});
(glob.sync('resources/plugins/custom/**/*.scss') || []).forEach(file => {
    mix.sass(file, CompilerPath + `${file.replace('resources/', '').replace('.scss', '.bundle.css')}`);
});

// wladi-workspace css pages (single page use)
//(glob.sync('resources/wladi-workspace/sass/pages/**/!(_)*.scss') || []).forEach(file => {
//    file = file.replace(/[\\\/]+/g, '/');
//    mix.sass(file, file.replace('resources/wladi-workspace/sass', CompilerPath +'css').replace(/\.scss$/, '.css'));
//});

// wladi-workspace js pages (single page use)
(glob.sync('resources/wladi-workspace/js/pages/**/*.js') || []).forEach(file => {
    mix.js(file, CompilerPath + `${file.replace('resources/wladi-workspace/', '')}`);
});

// wladi-workspace media
mix.copyDirectory('resources/wladi-workspace/media', CompilerPath + 'media');

// wladi-workspace theme
//(glob.sync('resources/wladi-workspace/sass/themes/**/!(_)*.scss') || []).forEach(file => {
//    file = file.replace(/[\\\/]+/g, '/');
//    mix.sass(file, file.replace('resources/wladi-workspace/sass', CompilerPath + 'css').replace(/\.scss$/, '.css'));
//});
...
mix.webpackConfig({
    plugins: [
        new ReplaceInFileWebpackPlugin([
            {
                // rewrite font paths
                dir: path.resolve(CompilerPath + 'plugins/global'),
                test: /\.css$/,
                rules: [
                    {
                        // fontawesome
...
(glob.sync('resources/wladi-workspace/plugins/**/*.+(woff|woff2|eot|ttf)') || []).forEach(file => {
    var folder = file.match(/resources\/wladi-workspace\/plugins\/(.*?)\//)[1];
    mix.copy(file, CompilerPath + `plugins/global/fonts/${folder}/${path.basename(file)}`);
});
...
], CompilerPath + 'plugins/custom/datatables/datatables.bundle.js');
mix.styles([
...
], CompilerPath + 'plugins/custom/datatables/datatables.bundle.css');
